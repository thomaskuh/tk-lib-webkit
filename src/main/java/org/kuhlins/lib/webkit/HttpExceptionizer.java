...
import org.kuhlins.lib.webkit.ex.model.ErrorResponse;

public class HttpExceptionizer {

    private static final Logger L = LogManager.getLogger(HttpExceptionizer.class);
    ...
    public void handleException(Exception e, HttpServletResponse resp) throws IOException {
        ...
        if (msgResolver != null) {
            ...
            er.getDetails()
                    .forEach(ed -> ed.setMsg(msgResolver.resolveClientExceptionMsg(ed.getKey(), ed.getParams())));
        }
